...
foreach ($pf in @(
  "kubectl -n $MonitoringNs port-forward svc/kpstack-grafana $GFPort:80",
  "kubectl -n $MonitoringNs port-forward svc/kpstack-kube-prometheus-st-prometheus $PMPort:9090",
  "kubectl -n $MonitoringNs port-forward svc/kpstack-kube-prometheus-st-alertmanager $AMPort:9093"
)) { try { Start-Process -WindowStyle Hidden -PassThru powershell -ArgumentList $pf | Out-Null } catch {} }
Start-Sleep 3

# --- HEALTH CHECKS (fix : pas de pipe après catch)
"/-/healthy","/-/ready","/api/v1/status/buildinfo" | ForEach-Object {
  $msg=""
  try   { iwr "http://localhost:$PMPort$_" -UseBasicParsing | Out-Null; $msg="Prometheus $_ => OK" }
  catch { $msg="Prometheus $_ => KO: $($_.Exception.Message)" }
  Write-Host $msg
}
foreach ($u in @("http://localhost:$GFPort/api/health","http://localhost:$AMPort/-/healthy")) {
  try { (iwr $u -UseBasicParsing).Content | Out-Null; Write-Host "$u => OK" }
  catch { Write-Host "$u => KO: $($_.Exception.Message)" }
}
...
if (-not $rendererOK) {
  Write-Host ">>> Activation renderer" -ForegroundColor Yellow
...
if (-not $promDs) { throw "Datasource Prometheus introuvable." }
$DsUid = $promDs.uid
...
$panels = @(
  ...
  @{ id=2; type="timeseries"; title="Latence p95 (s) - global"; gridPos=@{x=12;y=0;w=12;h=8};
     targets=@(@{refId="A"; datasource=@{type="prometheus";uid=$DsUid}; expr='histogram_quantile(0.95, sum by (le) (rate(http_request_duration_seconds_bucket{route!="/metrics"}[5m])))'; legendFormat='p95'}); fieldConfig=@{defaults=@{unit="s"}} },
  ...
  @{ id=5; type="timeseries"; title="Mémoire Pods (working set)"; gridPos=@{x=0;y=16;w=12;h=8};
     targets=@(@{refId="A"; datasource=@{type="prometheus";uid=$DsUid}; expr='max by (pod) (container_memory_working_set_bytes{pod=~"doctrine-demo.*"})'; legendFormat='{{pod}}'}); fieldConfig=@{defaults=@{unit="bytes"}} },
...
$resp = Invoke-WebRequest -Uri "http://localhost:$GFPort/api/dashboards/db" -Method POST -Headers $GFHeaders -ContentType "application/json" -Body $dashboard -UseBasicParsing
$FinalUid = ($resp.Content | ConvertFrom-Json).uid; if (-not $FinalUid) { $FinalUid = $DashUid }

# --- Simu courte (remplit les graphes)
if ($Simulate) {
  Write-Host ">>> Simulation trafic/erreurs/CPU/crash" -ForegroundColor Yellow
  ...
  Start-Sleep 2
  1..500 | % { Start-Job { iwr http://localhost:18080/healthz -UseBasicParsing | Out-Null } } | Out-Null
  Start-Sleep 3
  ...
  try { $pod = kubectl -n $AppNs get pod -l app=doctrine-demo -o jsonpath="{.items[0].metadata.name}"; if ($pod) { kubectl -n $AppNs delete pod $pod --grace-period=0 --force | Out-Null } } catch {}
}

# --- Rendu PNG (évite $PID qui est réservé)
Write-Host ">>> Rendu Grafana (PNG)" -ForegroundColor Cyan
$Images=@()
foreach ($panelId in 1..6) {
  ...
  try { Invoke-WebRequest -Uri $url -OutFile $out -Headers $GFHeaders -UseBasicParsing -TimeoutSec 40; $Images += $out; " rendu -> $out" | Write-Host }
  catch { Write-Warning "Echec rendu panel $panelId : $($_.Exception.Message)" }
}

# --- Prometheus -> CSV
function Get-Range($expr,$step="30s"){ $qe=[uri]::EscapeDataString($expr); $start=(Get-Date).AddHours(-8).ToUniversalTime().ToString("o"); $end=(Get-Date).ToUniversalTime().ToString("o"); $url="http://localhost:$PMPort/api/v1/query_range?query=$qe&start=$start&end=$end&step=$step"; ((iwr $url -UseBasicParsing).Content|ConvertFrom-Json).data.result }
function Summ($s){ $label=($s.metric|gm -MemberType NoteProperty|% Name|%{"$_=$($s.metric.$_)"})-join","; $vals=@(); foreach($v in $s.values){ $d=[double]$v[1]; if(-not[double]::IsNaN($d)){$vals+=$d} }; if($vals.Count-eq 0){[pscustomobject]@{series=$label;last=$null;max=$null;avg=$null}} else {[pscustomobject]@{series=$label;last=[Math]::Round($vals[-1],4);max=[Math]::Round(($vals|Measure-Object -Maximum).Maximum,4);avg=[Math]::Round(($vals|Measure-Object -Average).Average,4)} } }
...
(Get-Range $Q_RPS)|%{Summ $_}|Export-Csv -NoTypeInformation -Encoding UTF8 (Join-Path $OutRoot "rps.csv")
(Get-Range $Q_P95)|%{Summ $_}|Export-Csv -NoTypeInformation -Encoding UTF8 (Join-Path $OutRoot "p95.csv")
(Get-Range $Q_5XX)|%{Summ $_}|Export-Csv -NoTypeInformation -Encoding UTF8 (Join-Path $OutRoot "5xx.csv")
(Get-Range $Q_CPU)|%{Summ $_}|Export-Csv -NoTypeInformation -Encoding UTF8 (Join-Path $OutRoot "cpu.csv")
(Get-Range $Q_MEM)|%{Summ $_}|Export-Csv -NoTypeInformation -Encoding UTF8 (Join-Path $OutRoot "mem.csv")

# --- Snap JSON
try { (iwr "http://localhost:$PMPort/api/v1/targets" -UseBasicParsing).Content | Out-File (Join-Path $OutRoot "targets.json") -Encoding UTF8 } catch {}
try { (iwr "http://localhost:$AMPort/api/v2/alerts"  -UseBasicParsing).Content | Out-File (Join-Path $OutRoot "alerts.json")  -Encoding UTF8 } catch {}
...
<div class='grid'>
$(HtmlTable "RPS par route (avg/max/last)" (Import-Csv (Join-Path $OutRoot "rps.csv")))
$(HtmlTable "Latence p95 (global)"        (Import-Csv (Join-Path $OutRoot "p95.csv")))
$(HtmlTable "Erreurs 5xx par route"       (Import-Csv (Join-Path $OutRoot "5xx.csv")))
$(HtmlTable "CPU (pods app)"              (Import-Csv (Join-Path $OutRoot "cpu.csv")))
$(HtmlTable "Mémoire (pods app)"          (Import-Csv (Join-Path $OutRoot "mem.csv")))
</div>
...
function Print-Pdf($html,$pdf){
  ...
  else { Write-Warning "Chrome/Edge introuvable pour export PDF." }
}
