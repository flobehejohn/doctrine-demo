...
function Ensure-PF($name, $svc, $portPair, $healthUrl) {
  ...
  Start-Process -WindowStyle Hidden powershell -ArgumentList "kubectl -n $Ns port-forward svc/$svc $portPair" | Out-Null
  Start-Sleep 2
  if (!(Wait-HTTP $healthUrl)) {
    ...
    if ($pod) {
      Start-Process -WindowStyle Hidden powershell -ArgumentList "kubectl -n $Ns port-forward pod/$pod $portPair" | Out-Null
      Start-Sleep 2
    }
  }
